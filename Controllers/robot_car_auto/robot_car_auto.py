...
class  RobotCar () : 
    ...
    def  calcSteeringAngle (self,image) : 
        ...
        sumx = 0                   # X of yellow pixel Sum of coordinates 
        ...
        for y in range(int( 1.0 *self.camera_height/ 3.0 ),self.camera_height):
             for x in range( 0 ,self.camera_width):
                 if self.colorDiff( image[y,x], YELLOW) < 30 : # yellow range
                    sumx += x                       # Sum of x coordinates of yellow pixels 
                    pixel_count += 1                # Sum of number of yellow pixels
   
        # If no yellow pixels were detected, no pixels was detected... 
        if pixel_count == 0 :
             return self.UNKNOWN
        else :
